export const lyrics = [
  'Wet-ass pussy, make that pullout game weak',
  'make that pullout game weak',
  'Bring a bucket and a mop',
  'Bring a bucket and a mop for this wet-ass pussy',
  'Give me everything you got',
  'Give me everything you got for this wet-ass pussy',
  'Beat it up, nigga',
  'catch a charge',
  'Put this pussy right in your face',
  'Swipe your nose like a credit card',
  'Hop on top',
  'I wanna ride',
  "do a kegel while it's inside",
  'Spit in my mouth',
  'look in my eyes',
  'come take a dive',
  "Tie me up like I'm surprised",
  "Let's roleplay, I'll wear a disguise",
  'I want you to park that big Mack truck right in this little garage',
  'Make it cream',
  'make me scream',
  'make a scene',
  'Gobble me',
  'swallow me',
  'drip down the side of me',
  "Quick, jump out 'fore you let it get inside of me",
  'tell him where to put it',
  "never tell him where I'm 'bout to be",
  "run down on him 'fore I have a nigga runnin' me",
  'Talk your shit',
  'bite your lip',
  'Ask for a car while you ride that dick',
  'Now get your boots and your coat',
  'Now get your boots and your coat for this wet-ass pussy (Ah, ah, ah)',
  'Pay my tuition just to kiss me',
  'Pay my tuition just to kiss me on this wet-ass pussy (Mwah, mwah, mwah)',
  'Now make it rain if you wanna see some wet-ass pussy (Yeah, yeah)',
  'Now make it rain',
  'hope it lean over',
  'let him taste it, now he diabetic',
  "touch that lil' dangly thing that swing in the back of my throat",
  'punani Dasani',
  'ride on that thing',
  'ride on that thing like the cops is behind me',
  'spit on his mic',
  "Your honor, I'm a freak bitch, handcuffs, leashes",
  'Switch my wig',
  "make him feel like he cheatin'",
  'Put him on his knees',
  "give him somethin' to believe in",
  "I'm lookin' for a beatin'",
  "I'm the one that eat ya",
  "If he ate my ass, he's a bottom-feeder",
  "When I ride the dick, I'ma spell my name",
  'Bring a bucket and a mop for this wet-ass pussy',
  'Bring a bucket and a mop',
  'Give me everything you got for this wet-ass pussy',
  'Give me everything you got',
  "Now from the top, make it drop, that's some wet-ass pussy",
  'Now from the top, make it drop',
  'make it drop',
  "Now get a bucket and a mop, that's some wet-ass pussy",
  'Now get a bucket and a mop',
  "talkin' wap, wap, wap, that's some wet-ass pussy",
  "talkin' wap, wap, wap",
  "Macaroni in a pot, that's some wet-ass pussy, huh",
  'Macaroni in a pot',
]
